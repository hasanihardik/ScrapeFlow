...
const TaskMenu = () => {
  return (
    <aside className="flex-2 w-[340px] min-w-[340px] max-w-[340px]  border-r-2 border-separate h-full p-2 px-4 overflow-auto">
      <Accordion
        type="multiple"
        className="w-full"
        defaultValue={[
...
const TaskMenuBtn = ({ taskType }: { taskType: TaskType }) => {
  const { Icon, label } = TaskRegistry[taskType];
  const onDragStart = (e: React.DragEvent, type: TaskType) => {
    e.dataTransfer.setData("application/reactflow", type);
    e.dataTransfer.effectAllowed = "move";
  };
  const credit = TaskRegistry[taskType].credits;
  return (
    <Button
      variant="secondary"
      draggable
